...
class Employee:
    def __init__(self, name, family, manager=None):
        ...
        self.salary = 2500
        self.First_ever_family = self._family.copy()
    @property
    def id(self) -> int:
        return self._id
    @id.setter
    def id(self,new_id):
        raise Exception("Write acces to id is denied")
    @property
    def family(self) -> dict:
        return self.First_ever_family
    @family.setter
    def family(self, other):
        raise Exception("Write access to family is denied")   
    def apply_raise(self, managed_employee: 'Employee', raise_percent: int):
        if managed_employee._manager != self:
            raise Exception('You are not the manager, access denied')
        else:
            managed_employee.salary = (managed_employee.salary * raise_percent / 100) + managed_employee.salary
            return True
